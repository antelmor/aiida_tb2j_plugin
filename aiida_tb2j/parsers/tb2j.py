...
def correct_content(content):
    ...
    if content['colinear']:
        ...
        for data_type in shape:
            content[data_type].update({((0, 0, 0), i, i): np.zeros(shape[data_type]) for i in range(n)})
...
def branched_keys(tb2j_keys, npairs):
    ...
    return [new_keys[i:i+branch_size] for i in range(0, len(new_keys), branch_size)]
...
class TB2JParser(Parser):
    ...
    def _get_pickle_content(self, filename):

        import os, pickle

        pickle_path = os.path.join(self.retrieved._repository._get_base_folder().abspath, filename)
